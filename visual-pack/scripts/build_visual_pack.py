...
def discover(cfg, repo_root: Path):
        ...
        for rx in pats:
            for m in rx.finditer(t):
                ...
                if (r.startswith("/") and "*" not in r and ":" not in r
                        and not re.search(r"\.(json|png|jpg|svg|ico|txt|xml|webp)$", r)):
                    found.add(r)
    routes = sorted(found)
    ...
    with sync_playwright() as pw:
        ...
        # Reachability pre-check: fail fast with a clear message if nothing is served.
        try:
            page.goto(cfg["baseUrl"], wait_until="domcontentloaded", timeout=10000)
        except Exception as e:
            b.close()
            print(red(f"  ✘ Cannot reach {cfg['baseUrl']} — {type(e).__name__}"))
            print(yellow("    Start the server in another window (pnpm run preview),"))
            print(yellow("    then check the port it prints matches baseUrl in pack.config.json.\n"))
            return
        ...
        for r in routes:
            try:
                ...
                msg = str(e).split("\n")[0][:90]
                print(f"  {r:<34} {red(type(e).__name__)} {dim(msg)}")
        b.close()
