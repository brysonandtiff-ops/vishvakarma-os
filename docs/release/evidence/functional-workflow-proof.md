...
| Blueprint Editor: select tool, draw wall, add opening, inspect properties | e2e/editor-features.spec.ts (wall/door tool, wall select → properties), e2e/ipad-editor-layout.spec.ts | Sample project walls/openings, tool rail, properties panel on selection, 3D toggle, export dialog | PASS |
| Save/load/export/import preserves project data | e2e/editor-features.spec.ts, save-load-proof.md, import/export unit tests | Local draft reload PASS; save/load toasts + loading overlay; cloud reload PARTIAL until Supabase live proof | PARTIAL |
...
| Release Center and Audit Log show meaningful empty/loading states | e2e/governance-smoke.spec.ts, ReleasesPage + AuditLogPage guided empty CTAs | Release verification snapshot + audit/editor/spec-center empty-state actions | PASS |
