...
|---------|----------------|-----------|
| **Cloudflare Pages** | Cloudflare dashboard | Deploy failures, Function errors, traffic |
...
`VITE_SENTRY_DSN` is set. Set `VITE_SENTRY_DSN` in the Cloudflare Pages production environment to
...
### Cloudflare Web Analytics — activation funnel
...
Cloudflare Web Analytics dashboard. (Custom events require a Cloudflare Web Analytics paid plan; without
...
|--------|-------------|--------|
| Cloudflare Pages function 5xx rate | Sustained > 1% | Check logs; consider rollback |
...
| Component | Logs |
|-----------|------|
| Cloudflare Pages Functions | Cloudflare Pages → Functions → Logs |
