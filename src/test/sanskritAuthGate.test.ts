...
describe('Sanskrit auth gate design', () => {
  ...
  it('shows the divine login mockup with Supabase auth flows', () => {
    ...
    expect(loginCard).toContain('Send magic link');
    expect(loginCard).toContain('secure email access link or continue with Google OAuth');
    expect(loginCard).toContain('onMagicLink');
