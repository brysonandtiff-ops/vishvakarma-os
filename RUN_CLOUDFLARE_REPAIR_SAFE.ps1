...
function Get-WorktreeEntries {
    ...
    foreach ($Line in $Lines) {
        ...
        [void]$Entries.Add([pscustomobject]@{
            ...
            IsGenerated = Test-IsGeneratedPath $PathText
        })
    }
...
function Write-AutopilotResult {
    ...
    $Payload = [pscustomobject]@{
        ...
        GeneratedBackup = if (Test-Path $BackupRoot) { $BackupRoot } else { $null }
    }
...
# Clear stale index flags from any interrupted older run before inspecting the tree.
foreach ($Path in @(Get-TrackedGeneratedPaths)) {
    git update-index --no-skip-worktree -- $Path 2>$null
    git update-index --no-assume-unchanged -- $Path 2>$null
}
...
try {
    ...
    Write-Host "Mode: $(if ($NonInteractive) { 'unattended vault/session reuse' } else { 'automatic with one-time secure prompts when required' })" -ForegroundColor Yellow
    ...
    for ($Attempt = 1; $Attempt -le $MaxAttempts; $Attempt++) {
        ...
        $FinalSummaryPath = if ($Summary) { $Summary.Path } else { $null }
