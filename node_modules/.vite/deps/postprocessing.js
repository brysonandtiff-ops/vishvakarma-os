import { $ as CanvasTexture, Br as LoadingManager, Ci as NoBlending, Ds as Vector3, Es as Vector2, F as BackSide, Fr as LinearSRGBColorSpace, Gr as Material, Gt as DepthStencilFormat, I as BasicDepthPacking, In as HalfFloatType, Is as WebGLRenderTarget, It as Data3DTexture, K as BufferAttribute, Kt as DepthTexture, Nr as LinearMipmapLinearFilter, Os as Vector4, Qr as Mesh, Qt as DoubleSide, Rr as Loader, Rt as DataTexture, Sn as FrontSide, Ui as OrthographicCamera, Xr as Matrix4, Ya as RGFormat, Zo as Texture, _s as UnsignedByteType, ei as MeshDepthMaterial, fn as EventDispatcher, fo as SRGBColorSpace, g as AlwaysDepth, gr as LessEqualDepth, ha as RGBAFormat, hn as FileLoader, ho as ShaderMaterial, ii as MeshNormalMaterial, it as ClampToEdgeWrapping, jn as GreaterEqualDepth, ki as NotEqualDepth, kn as GreaterDepth, kr as LinearFilter, ma as RGBADepthPacking, mi as NearestFilter, mo as Scene, mr as LessDepth, ms as Uniform, oo as RepeatWrapping, ot as Color, pa as REVISION, q as BufferGeometry, qi as PerspectiveCamera, sn as EqualDepth, to as RedFormat, vn as FloatType, wi as NoColorSpace, xi as NeverDepth, ys as UnsignedInt248Type } from "./three.module-BC2mUsB1.js";

//#region node_modules/.pnpm/postprocessing@6.39.1_three@0.180.0/node_modules/postprocessing/build/index.js
/**
	...
	*/
	initialize(renderer, alpha, frameBufferType) {
		if (frameBufferType !== void 0) {
			...
			if (frameBufferType !== UnsignedByteType) this.fullscreenMaterial.defines.FRAMEBUFFER_PRECISION_HIGH = "1";
			else if (renderer !== null && renderer.outputColorSpace === SRGBColorSpace) this.renderTarget.texture.colorSpace = SRGBColorSpace;
		}
	}
};
...
var EffectComposer = class {
	...
	*/
	createBuffer(depthBuffer, stencilBuffer, type, multisampling) {
		...
		if (type === UnsignedByteType && renderer !== null && renderer.outputColorSpace === SRGBColorSpace) renderTarget.texture.colorSpace = SRGBColorSpace;
	...
	*/
	constructor(colorOutput = false, luminanceRange = null) {
		super({
			...
			defines: { THREE_REVISION: REVISION.replace(/\D+/g, "") },
			uniforms: {
				inputBuffer: new Uniform(null),
	...
	*/
	initialize(renderer, alpha, frameBufferType) {
		if (frameBufferType !== void 0) {
			...
			} else if (renderer !== null && renderer.outputColorSpace === SRGBColorSpace) for (const mipmap of mipmaps) mipmap.texture.colorSpace = SRGBColorSpace;
		}
	}
	/**
	* Deletes internal render targets and textures.
...
var MaskMaterial = class extends ShaderMaterial {
	...
	*/
	set maskTexture(value) {
		...
		if (value.type !== UnsignedByteType) this.defines.MASK_PRECISION_HIGH = "1";
		this.needsUpdate = true;
	}
	/**
...
var ShaderPass = class extends Pass {
	...
	*/
	initialize(renderer, alpha, frameBufferType) {
		if (frameBufferType !== void 0 && frameBufferType !== UnsignedByteType) this.fullscreenMaterial.defines.FRAMEBUFFER_PRECISION_HIGH = "1";
	}
};
...
var LUT1DEffect = class extends Effect {
	...
	*/
	...
	set lut(value) {
		...
		if (value !== null && (value.type === FloatType || value.type === HalfFloatType)) this.defines.set("LUT_PRECISION_HIGH", "1");
	}
};
...
var LookupTexture = class _LookupTexture extends Data3DTexture {
	...
	*/
	applyLUT(lut) {
		...
		else if (lut.type !== FloatType || this.type !== FloatType) console.error("Both LUTs must be FloatType textures");
		else if (lut.format !== RGBAFormat || this.format !== RGBAFormat) console.error("Both LUTs must be RGBA textures");
		else {
	...
	set lut(value) {
		...
		if (this.lut !== value) {
			...
			if (value !== null) {
				...
				if (value.type === FloatType || value.type === HalfFloatType) defines.set("LUT_PRECISION_HIGH", "1");
...
var SelectiveBloomEffect = class extends BloomEffect {
	...
	*/
	initialize(renderer, alpha, frameBufferType) {
		...
		if (frameBufferType !== void 0) {
			this.renderTargetMasked.texture.type = frameBufferType;
			if (renderer !== null && renderer.outputColorSpace === SRGBColorSpace) this.renderTargetMasked.texture.colorSpace = SRGBColorSpace;
		}
	}
};
	...
	set texture(value) {
		...
		if (prevTexture !== value) {
			...
			if (value !== null) {
				...
				if (value.type !== UnsignedByteType) defines.set("TEXTURE_PRECISION_HIGH", "1");
...
var ToneMappingEffect = class extends Effect {
	...
	*/
	...
	set mode(value) {
		...
		const cineonToneMapping = REVISION.replace(/\D+/g, "") >= 168 ? "CineonToneMapping(texel)" : "OptimizedCineonToneMapping(texel)";
		this.defines.clear();
...
var DepthPickingPass = class extends DepthCopyPass {
	...
	*/
	constructor({ depthPacking = RGBADepthPacking, mode = DepthCopyMode.SINGLE } = {}) {
		if (depthPacking !== RGBADepthPacking && depthPacking !== BasicDepthPacking) throw new Error(`Unsupported depth packing: ${depthPacking}`);
		super({ depthPacking });
...
`;
		...
		if (mainImageExists) {
			if (effect.inputColorSpace !== null && effect.inputColorSpace !== data.colorSpace) fragmentMainImage += effect.inputColorSpace === SRGBColorSpace ? "color0 = sRGBTransferOETF(color0);\n	" : "color0 = sRGBToLinear(color0);\n	";
			if (effect.outputColorSpace !== NoColorSpace) data.colorSpace = effect.outputColorSpace;
...
var EffectPass = class extends Pass {
	...
	*/
	updateMaterial() {
		...
		if (data.colorSpace === SRGBColorSpace) fragmentMainImage += "color0 = sRGBToLinear(color0);\n	";
