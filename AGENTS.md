...
- **Verifying editor functionality:** The 2D blueprint canvas does **not** visibly paint wall/opening geometry in headless Chromium screenshots — the repo's own committed demo screenshots (`docs/demo/screenshots/03-*.png`) show an empty canvas with only a faint compass even when the status bar reports `Walls: 6`. Do **not** judge editor correctness by pixel-inspecting canvas screenshots. Instead verify via the `.ws-status-bar` `Walls:`/`Openings:` counters and the E2E draw proof: `pnpm exec playwright test --config=playwright.deep-proof.config.ts` (draws a wall, places a door, asserts wall properties). Tools activate via the tool rail buttons or keyboard (`W` = Wall).
