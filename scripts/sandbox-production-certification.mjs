...
const candidateSha = 'c93a595028a014b06e4a1df0275042187c519ac7';
...
async function main() {
  ...
  console.log(`[production-cert] Candidate source SHA: ${candidateSha}`);
  console.log(`[production-cert] Certification runner SHA: ${runnerSha}`);
  ...
  try {
    ...
    const certify = [
      ...
      `node -e "const fs=require('fs'); fs.writeFileSync('docs/release/evidence/sandbox-production-certification-result.json', JSON.stringify({status:'PASS',candidateSha:'${candidateSha}',runnerSha:'${runnerSha}',completedAt:new Date().toISOString()},null,2)+'\\n')"`,
    ].join('; ');
    sandbox([
    ...
    console.log(`[production-cert] PASS — exact source certification completed for ${candidateSha}.`);
  } finally {
