...
const candidateSha = '6e0b9894fd875d98dbb20ba49e5a47a148558f33';
...
const sourceArchive = `/tmp/vish-exact-${phase}.tgz`;
const sandboxName = `vish-exact-${phase}-${Date.now()}`;
...
function run(program, args, { allowFailure = false, timeoutMs = 12 * 60_000 } = {}) {
  console.log(`[exact-cert:${phase}] ${[program, ...args].join(' ')}`);
  ...
  if (!allowFailure && result.status !== 0) {
    ...
    throw new Error(`${[program, ...args].join(' ')}${timedOut}; exit ${result.status}`);
  }
  return result;
}
...
async function main() {
  if (!command) throw new Error(`Unknown phase: ${phase || '(empty)'}`);
  ...
  try {
    sandbox([
      ...
      '--timeout', '35m', '--vcpus', '4', '--network-policy', 'allow-all',
    ], { timeoutMs: 4 * 60_000 });
    created = true;
    ...
    sandbox([
      'exec', '--sudo', '--timeout', '22m', sandboxName, '--',
      'chroot', '/opt/ubuntu', '/bin/bash', '-lc', certify,
    ], { timeoutMs: 24 * 60_000 });

    await writeResult({
      status: 'PASS', phase, candidateSha, runnerSha, snapshotId,
...
main().catch(async (error) => {
  await writeResult({
    status: 'FAIL', phase, candidateSha, runnerSha, snapshotId,
