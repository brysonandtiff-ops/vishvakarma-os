...
const sandboxName = `vish-every-page-audit-${Date.now()}`;
const oidcToken = process.env.VERCEL_OIDC_TOKEN?.trim();
const candidateSha = process.env.VERCEL_GIT_COMMIT_SHA?.trim() || 'unknown';
const sourceArchive = '/tmp/vish-every-page-source.tgz';
const resultRoot = '/tmp/vish-every-page-result';
const craneVersion = 'v0.20.3';

function run(program, args, { allowFailure = false, timeoutMs = 20 * 60_000 } = {}) {
  console.log(`[every-page-audit] ${[program, ...args].join(' ')}`);
...
async function createSourceArchive() {
  run('tar', [
    ...
    '--exclude=playwright-every-page-report',
...
async function writeFallback(error) {
  ...
  await writeFile(
    'dist/index.html',
    `<!doctype html><html><body><h1>Every-page audit error</h1><pre>${String(report.error)}</pre></body></html>`,
    'utf8',
  );
}
...
async function main() {
  ...
  try {
    ...
    await cp(resultRoot, 'dist', { recursive: true });
...
main().catch(async (error) => {
  console.error('[every-page-audit] ERROR:', error);
  await writeFallback(error);
});
