...
function run(program, args, { allowFailure = false, timeoutMs = 12 * 60_000 } = {}) {
  ...
  const result = spawnSync(program, args, {
    cwd: process.cwd(),
    env: { ...process.env, VERCEL_TOKEN: oidcToken ?? '', VERCEL_TEAM_ID: teamId, VERCEL_PROJECT_ID: projectId },
    encoding: 'utf8',
  ...
  if (!allowFailure && result.status !== 0) {
    throw new Error(`${[program, ...args].join(' ')}; exit ${result.status}${result.error?.message ? `: ${result.error.message}` : ''}`);
  }
  return result;
}
...
async function readRemoteExitCode() {
  ...
  for (let attempt = 1; attempt <= 18; attempt += 1) {
    const copied = sandbox(['copy', `${sandboxName}:${remoteExitMarker}`, localExitMarker], { allowFailure: true, timeoutMs: 90_000 });
    if (copied.status === 0) {
...
async function main() {
  ...
  run('tar', ['--exclude=.git', '--exclude=node_modules', '--exclude=dist', '--exclude=coverage', '--exclude=playwright-report', '--exclude=test-results', '--exclude=.vercel', '-czf', sourceArchive, '.'], { timeoutMs: 4 * 60_000 });
  ...
  try {
    ...
    const refresh = ['set -euo pipefail', 'mkdir -p /opt/ubuntu/proc /opt/ubuntu/sys /opt/ubuntu/dev/pts /opt/ubuntu/app', 'mountpoint -q /opt/ubuntu/proc || mount -t proc proc /opt/ubuntu/proc', 'mountpoint -q /opt/ubuntu/sys || mount --rbind /sys /opt/ubuntu/sys', 'mount --make-rslave /opt/ubuntu/sys || true', 'mountpoint -q /opt/ubuntu/dev || mount --rbind /dev /opt/ubuntu/dev', 'mount --make-rslave /opt/ubuntu/dev || true', 'cp --remove-destination /etc/resolv.conf /opt/ubuntu/etc/resolv.conf', 'cp --remove-destination /etc/hosts /opt/ubuntu/etc/hosts', "find /opt/ubuntu/app -mindepth 1 -maxdepth 1 ! -name node_modules -exec rm -rf {} +", 'tar -xzf /tmp/source.tgz -C /opt/ubuntu/app'].join('; ');
    sandbox(['exec', '--sudo', '--timeout', '4m', sandboxName, '--', 'bash', '-lc', refresh], { timeoutMs: 5 * 60_000 });

    const certify = ['set +e', `rm -f /tmp/vish-v1.5-${phase}.exit`, '( set -euo pipefail', 'cd /app', 'corepack enable', 'corepack prepare pnpm@9.15.0 --activate', 'pnpm install --frozen-lockfile', 'export CI=1', 'export VITE_E2E_ALLOW_LOCAL_ACCESS=true', 'export VITE_ALLOW_LOCAL_DEMO=true', 'export PRODUCTION_AUTH_URL=https://vishvakarma-os.app/auth', command, ')', 'status=$?', `printf "%s\\n" "$status" > /tmp/vish-v1.5-${phase}.exit`, 'exit "$status"'].join('; ');
    const transport = sandbox(['exec', '--sudo', '--timeout', '26m', sandboxName, '--', 'chroot', '/opt/ubuntu', '/bin/bash', '-lc', certify], { allowFailure: true, timeoutMs: 28 * 60_000 });
    const remoteExitCode = await readRemoteExitCode();
    if (remoteExitCode !== 0) throw new Error(`Certification phase ${phase} failed with remote exit ${remoteExitCode}; transport exit ${transport.status}`);

    await writeResult({ status: 'PASS', phase, candidateSha, runnerSha, snapshotId, command, remoteExitCode, transportExitCode: transport.status, completedAt: new Date().toISOString() });
  } finally {
    if (created) cleanup();
...
main().catch(async (error) => {
  await writeResult({ status: 'FAIL', phase, candidateSha, runnerSha, snapshotId, command: command ?? null, error: error instanceof Error ? error.message : String(error), completedAt: new Date().toISOString() });
  await rm(sourceArchive, { force: true });
  await rm(localExitMarker, { force: true });
  process.exitCode = 1;
});
