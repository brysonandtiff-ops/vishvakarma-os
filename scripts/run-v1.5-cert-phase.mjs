...
const candidateSha = '8c607811f2ea9fe0f9ca1580feb9105b83aced27';
...
const sandboxName = `vish-v1-5-final-${phase}-${Date.now()}`;
...
async function main() {
  ...
  try {
    ...
    if (remoteExitCode !== 0) {
      throw new Error(
        `Certification phase ${phase} failed with remote exit ${remoteExitCode}; transport exit ${transport.status}`,
      );
    }
